...
func checkInvalidTx(t *testing.T, anteHandler sdk.AnteHandler, ctx sdk.Context, tx sdk.Tx, simulate bool, code sdk.CodeType) {
	...
	if code == sdk.CodeOutOfGas {
		...
		require.Equal(t, stdTx.GetFee().GasWanted, result.GasWanted, "Gas wanted not set correctly")
...
func TestAnteHandlerSigErrors(t *testing.T) {
	...
	// removed by junying, 2019-11-14
	// that's because dk how no-sig tx generated on earth
	// test no signatures
	...
	tx = newTestTx(ctx, msgs[0], privs[1], accNums[0], seqs[0])
...
func TestAnteHandlerAccountNumbers(t *testing.T) {
	...
	tx = newTestTx(ctx, msgs[0], privs[0], accnums[0], seqs[0])
	checkValidTx(t, anteHandler, ctx, tx, false)

	// new tx from wrong account number
	tx = newTestTx(ctx, msgs[0], privs[0], uint64(1), uint64(1))
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeUnauthorized)

	// from correct account number
	tx = newTestTx(ctx, msgs[0], privs[0], uint64(0), uint64(1))
	checkValidTx(t, anteHandler, ctx, tx, false)
...
// 	tx = newTestTx(ctx, msgs, []crypto.PrivKey{priv2}, []uint64{1}, []uint64{1})
// 	checkValidTx(t, anteHandler, ctx, tx, false)
...
// 	tx = newTestTx(ctx, []sdk.Msg{msg}, privs, accnums, seqs)
...
// 	tx = newTestTx(ctx, []sdk.Msg{msg1}, []crypto.PrivKey{priv1, priv2}, []uint64{0, 1}, []uint64{1, 1})
// 	checkValidTx(t, anteHandler, ctx, tx, false)
// 	tx = newTestTx(ctx, []sdk.Msg{msg2}, []crypto.PrivKey{priv3, priv1}, []uint64{2, 0}, []uint64{1, 2})
// 	checkValidTx(t, anteHandler, ctx, tx, false)
...
// 	msg = newTestMsg(addr2)
// 	msgs = []sdk.Msg{msg}
// 	tx = newTestTx(ctx, msgs, privs, []uint64{1}, seqs)
